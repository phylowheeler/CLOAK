...
```
Alternatively, the user can run CLOAK directly within Muscle5 with the -cloak option, with the user providing either a multiple alignment file or directory of variant alignments. In this form, it is still recommended to use an ensemble fasta file generated by muscle5 with the command above as the input. 
```
